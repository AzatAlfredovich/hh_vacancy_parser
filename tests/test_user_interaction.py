...
def test_user_interaction(monkeypatch):
    ...
    # Вариант 1 (предпочтительно): патчим именно то имя, которое использует user_interaction.py
    monkeypatch.setattr(
        "src.user_interaction.export_to_json", _fake_export_to_json, raising=False
    )
