...
def user_interaction(hh_api, saver):
    ...
    vacancies_data = hh_api.load_vacancies(
        keyword
    )  # Получаем список словарей с вакансиями
    ...
    vacancies = [
        Vacancy(
            ...
            extract_salary(
                v.get("salary")
            ),  # Передаем числовую зарплату из extract_salary
    ...
    filtered.sort(
        key=lambda v: v.salary if isinstance(v.salary, int) else 0, reverse=True
    )
    ...
    export_format = (
        input("В каком формате хотите сохранить вакансии? (json/csv/нет): ")
        .strip()
        .lower()
    )
